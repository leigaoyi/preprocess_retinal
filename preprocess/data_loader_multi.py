...
# process RGB
#from .pre_deep import my_preprocess
from .pre_multi import my_preprocess
...
class MyImageItem(torch.utils.data.Dataset):
    ...
    def __getitem__(self, item):
        ...
        img_patches_norm = np.reshape(img_patches_norm, [num_patches, patch_size, patch_size, 3])
       # print('***********')
...
def extract_random_patches(img, label, num_patches=20, patch_size=48):
    ...
    img_patches = np.reshape(img_patches, [num_patches, patch_size, patch_size, 3])
...
class MyTestItem(torch.utils.data.Dataset):    
    ...
    def __getitem__(self, item):
        ...
        img_patches = np.asarray(img_norm, np.float32)
        img_patches = extract_whole_patches(img_patches)
        img_patches_norm = img_patches / 255.
